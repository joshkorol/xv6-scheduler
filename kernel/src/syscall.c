...
fetchint(uint addr, int *ip)
{
  ...
  if(addr >= curproc->owner->sz || addr+4 > curproc->owner->sz) {
...
fetchstr(uint addr, char **pp)
{
  ...
  struct proc *curproc = myproc();
  acquire(&myproc()->owner->grouplock);
  if(addr >= curproc->owner->sz) {
    release(&myproc()->owner->grouplock);
    return -1;
  }
  *pp = (char*)addr;

  ep = (char*)curproc->owner->sz;
  for(s = *pp; s < ep; s++){
    if(*s == 0) {
      release(&myproc()->owner->grouplock);
      return s - *pp;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  acquire(&myproc()->owner->grouplock);
  if(size < 0 || (uint)i >= curproc->owner->sz || (uint)i+size > curproc->owner->sz) {
    release(&myproc()->owner->grouplock);
    return -1;
  }
  release(&myproc()->owner->grouplock);
